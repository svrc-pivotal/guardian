...
func NewGardenRunner(bin, initBin, nstarBin, containerdBin, dadooBin, grootfsBin, rootfs, tarBin, network, address string, argv ...string) GardenRunner {
	r := GardenRunner{}
	...
	r.Cmd = cmd(r.TmpDir, r.DepotDir, r.GraphPath, r.Network, r.Addr, bin, initBin, nstarBin, containerdBin, dadooBin, grootfsBin, tarBin, rootfs, argv...)
...
func Start(bin, initBin, nstarBin, containerdBin, dadooBin, grootfsBin, rootfs, tarBin string, argv ...string) *RunningGarden {
	runner := NewGardenRunner(bin, initBin, nstarBin, containerdBin, dadooBin, grootfsBin, rootfs, tarBin, "unix", fmt.Sprintf("/tmp/garden_%d.sock", GinkgoParallelNode()), argv...)
...
func cmd(tmpdir, depotDir, graphPath, network, addr, bin, initBin, nstarBin, containerdBin, dadooBin, grootfsBin, tarBin, rootfs string, argv ...string) *exec.Cmd {
	...
	gardenArgs = appendDefaultFlag(gardenArgs, "--containerd-bin", containerdBin)
