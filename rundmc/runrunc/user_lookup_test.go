...
var _ = Describe("LookupUser", func() {
	...
	Context("when we try to get information for the empty username", func() {
...
_dovecot:*:214:6:Dovecot Administrator:/var/empty:/usr/bin/false`,
		...
		It("loads the information from /etc/passwd", func() {
		...
		Context("when a user has supplementary groups", func() {
			BeforeEach(func() {
				Expect(ioutil.WriteFile(filepath.Join(rootFsPath, "etc", "group"), []byte(
					`adm:x:4:syslog,devil`,
				), 0777)).To(Succeed())
			})

			It("loads the supplementary GIDs from /etc/group", func() {
				user, err := runrunc.LookupUser(rootFsPath, "devil")
				Expect(err).ToNot(HaveOccurred())
				Expect(user.Sgids).To(BeEquivalentTo([]int{4}))
			})
		})
	})
