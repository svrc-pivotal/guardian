...
var _ = Describe("NotImplementedCommandCreator", func() {
	...
	Describe("CreateCommand", func() {
		It("returns nil and provided error", func() {
			cmd, err := notImplementedCommandCreator.CreateCommand(nil, "", rootfs_provider.Spec{})
			Expect(cmd).To(BeNil())
	...
	Describe("GCCommand", func() {
		It("returns nil", func() {
			Expect(notImplementedCommandCreator.GCCommand(nil)).To(BeNil())
		})
	})
})
