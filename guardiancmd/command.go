...
func (cmd *GuardianCommand) wireNetworker(log lager.Logger, propManager kawasaki.ConfigStore, portPool *ports.PortPool) (gardener.Networker, gardener.Starter, error) {
	...
	//TODO: Why do we need to create another iptables.New for the default configurer
	kawasakiNetworker := kawasaki.New(
		...
		kawasaki.NewConfigCreator(idGenerator, interfacePrefix, chainPrefix, externalIP, dnsServers, cmd.Network.Mtu, cmd.Bin.IPTables.Path()),
		propManager,
		factory.NewDefaultConfigurer(iptables.New(cmd.Bin.IPTables.Path(), linux_command_runner.New(), chainPrefix)),
		portPool,
